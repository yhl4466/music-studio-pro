...
export function bindTopControls(){
  const syncBpm=v=>{proj.bpm=v;UI.bpm.value=v;UI.bpmNum.value=v;UI.posSub.textContent='BPM '+v+' · '+meterLabel();if(A&&A.dl)A.dl.delayTime.value=Math.min(1.5,barSeconds()/2)};
  ...
  // 菜单
  UI.menuBtn.addEventListener('click',e=>{e.stopPropagation();UI.menu.classList.toggle('open')});
...
export function syncBpmUI(){
  ...
  UI.posSub.textContent='BPM '+proj.bpm+' · '+meterLabel()+' · '+(proj.steps/SPB())+' 小节';
}
...
export function showHelp(){
  const content=`
    <div style="font-size:17px;font-weight:800;margin-bottom:2px">🎹 智能音乐工坊 PRO — 使用教程</div>
    ...
    <div class="small" style="line-height:1.9">
      ...
      ・ <b>🎵 节奏细分（连音）</b>：先用 🔲/Shift <b>框选整数拍</b>（起点对齐步 0/4/8/12…），再到顶部量化按钮旁选 <b>2/3/4/5/6 连音</b> 并点「♫ 应用细分」——选区里每拍的起音会变成 N 个<b>等长精确时值</b>（各 1/N 拍，2/3/4/5/6 全部无浮点误差），MIDI 导出按真实比例（3 连音 = 160 tick）；选「还原为网格」可回到普通格子（可 Ctrl+Z）。轨道页侧栏「♫ 节奏细分」是同一功能。<br>
      ・ <b>🔲 框选区域</b>：点 🔲 选区 后拖动；或<b>任意工具下按住 Shift 拖动</b>即可框选（状态条会显示范围与音符数，Esc 取消）。<br>
      ・ <b>📋 复制 / 📌 粘贴（Ctrl+C / Ctrl+V）</b>：框选一段 → 复制 → 把<b>播放头</b>移到目标轨起点（或按住 <b>Alt</b> 单击目标格直接定位）→ 粘贴；越界自动加小节，旋律/鼓不能混贴。<br>
      ・ <b>🎯 量化</b>：选 1/8 / 1/4 网格与强度后点「吸附」。<b>没有选区=整首量化；先用 🔲/Shift 框选一段=只量化那一段</b>。点吸附前会实时显示“预计移动 ~N 处”；完成后被移动/合并的音符会<b>闪绿</b>，结果条会说明“移动 N 处、去重合并 M 处”，可 Ctrl+Z 撤销。<br>
      ・ 顶部<b>进度条</b>点击/拖动可任意跳转；标尺也可拖动定位。<br>
      ・ <b>↩ 撤销 / ↪ 重做</b>（Ctrl+Z / Ctrl+Shift+Z / Ctrl+Y）覆盖大部分操作。
    </div>
    ...
    <div class="small" style="line-height:1.9">
      ...
      ・ 右上 <b>🎛 EQ</b>：主输出低频/中频/高频 ±12dB，外加<b>砖墙限制器</b>（可开关）防削波。
    </div>

    <div class="h2">④ 演奏</div>
    <div class="small" style="line-height:1.9">
      ・ 底部钢琴<b>按住可左右滑动连奏、松手释放</b>；电脑键盘 <kbd>A~L</kbd> 白键、<kbd>W E T Y U O P</kbd> 黑键，八度 ±。<br>
      ・ <b>⌖ 对准音区</b>把键盘起点对齐到所选旋律轨，方便跟弹；<kbd>X</kbd> 试听旋律轨音符。
    </div>

    <div class="h2">⑤ 保存 / 作品库 / 分享</div>
    <div class="small" style="line-height:1.9">
      ・ 菜单：导出工程(JSON)/导入、导出 <b>WAV / MIDI</b>、快速保存(Ctrl+S)；工程自动存档。<br>
      ・ 侧栏「<b>📚 作品库</b>」：把编好的曲子命名保存到浏览器，随时载入/复制/删除。<br>
      ・ 菜单「复制分享链接」：<b>自动压缩</b>，别人打开链接即可还原整首工程。
    </div>
    ...
    <div class="helpLine"><kbd>Shift+拖动 / 🔲 选区</kbd><span>框选一段（用于 复制 或 区域量化）</span></div>
    ...
    <div class="helpLine"><kbd>Esc</kbd><span>取消框选</span></div>
    ...
    <div style="margin-top:14px;text-align:right"><button class="btn acc" id="helpClose">开始创作 🎵</button></div>`;
  const ov=el('div','',`<div style="position:fixed;inset:0;background:rgba(2,5,12,.72);z-index:300;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(4px)">
    <div style="width:min(640px,94vw);max-height:88vh;overflow:auto;background:var(--panel);border:1px solid var(--line2);border-radius:var(--r);padding:24px;box-shadow:var(--shadow-3)">${content}</div>
  </div>`);
...
export function afterProjectLoad(){ structural(true);buildKeyBar();syncAllUI();renderInspector();relabelRows(); }
